...
const { queryType, mutationType, stringArg, makeSchema, objectType, nonNull, plugin, fieldAuthorizePlugin } = require('nexus');
    ...
    definition(t) {
        t.field('singleClient', {
            ...
            authorize: async (_, args, context) => {
                await authenticate(_, args, context);
                return true;
            },
            // authorize: async (_, args, context) =>await authenticate(_, args, context),
            resolve: (_, args) => {
        ...
        t.list.field('manyClients', {
            ...
            // authorize: async (_, args, context) =>authenticate(_, args, context),
            resolve: () => {
...
const authenticate = async (_, args, context) => {
    const token = context.req.headers.token;
    try {
        const decoded = jwt.verify(token, secretKey);
        return decoded;
        // write above return true; and can write authorize field as commented(eg below).
        // authorize: async (_, args, context) =>await authenticate(_, args, context),
    } catch (error) {
